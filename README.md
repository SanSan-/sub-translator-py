...
- Локальная проверка модели без сети и возобновляемая докачка в выбранный каталог после промаха
  кеша, если пользователь включил загрузку.
...
| `seedx` | локальный | Seed-X PPO 7B для приоритета качества EN↔RU | CUDA Int4; один текст за запрос процесса; скрытого перехода на CPU нет |
...
Профиль использует `ByteDance-Seed/Seed-X-PPO-7B-AWQ-Int4` с закреплённой ревизией и стандартным
каталогом `<repo-root>/models/seed-x-ppo-7b-awq-int4`. Уже полный каталог переиспользуется без
повторной загрузки. Модель работает в отдельном процессе на `cuda:0`; 4-битные сжатые веса
проверяются и загружаются через `compressed-tensors==0.18.0`. Размещение на CPU или диске и
сетевой переход при ошибке запрещены.
...
около 5 ГБ на диске и по 8 ГБ оперативной и видеопамяти. Лицензия профиля — OpenMDW.
...
│  │  └─ seedx_worker.py                # Seed-X compressed-tensors Int4 только в CUDA
│  ├─ utils
...
│  │  ├─ huggingface.py                 # проверка, докачка и подпись локальной модели
...
Явный `--model-path` не подменяется другим каталогом. Полный ручной каталог без служебной отметки
либо с корректной отметкой возвращается без импорта клиента Hugging Face и без сетевого запроса.
Повреждённая, чужая или устаревшая отметка отклоняется. Если каталога нет или он неполон, обычный
запуск завершается понятной ошибкой. Параметр `--auto-download-model` разрешает после промаха
кеша:

1. проверить закреплённую ревизию и список файлов;
2. оценить недостающий объём и свободное место с резервом;
3. получить межпроцессную блокировку каталога;
4. продолжить загрузку в тот же каталог;
5. повторно проверить модель и атомарно записать служебную отметку.
...
алгоритма. Для вручную подготовленного каталога модели дополнительно учитывается подпись его
содержимого. Значения секретов в подпись не входят.
...
```powershell
# Докачать NLLB в стандартный каталог после промаха кеша.
uv run --frozen --no-dev python -m sub_translate `
...
# Докачать экспериментальную TranslateGemma 12B в её стандартный каталог.
uv run --frozen --no-dev python -m sub_translate `
  --input "<subtitle-file>" --from en --to ru --api translategemma-12b `
  --auto-download-model `
  --worker-python-path "<worker-python-path>"

# Переиспользовать подготовленный каталог Seed-X и один текст за запрос процесса.
uv run --frozen --no-dev python -m sub_translate `
  --input "<subtitle-file>" --from en --to ru --api seedx `
  --model-path "<repo-root>\models\seed-x-ppo-7b-awq-int4" `
  --worker-python-path "<worker-python-path>" `
  --batch-size 1
```
...
- `--auto-download-model` — разрешение на докачку после промаха кеша;
...
перевода. Перетаскивание файлов намеренно не используется: путь возвращает системный диалог на
той же машине, поэтому содержимое субтитров не загружается через браузер.
...
- для Google появляются домен, время ожидания и задержка между запросами;
...
- для локального профиля появляются идентификатор и ревизия модели, каталог, разрешение на
  докачивание и явный переход NLLB на CPU;
...
считаются временными и туда не записываются. Токены Hugging Face и `OPENAI_API_KEY` отсутствуют в
HTML, моделях запросов и ответах API; интерфейс получает только логический признак готовности
ключа или токена на сервере.
...
| `POST /api/pick` | системный выбор файлов или каталога | режим выбора и подготовленные элементы файлов |
| `POST /api/refresh` | повторная проверка путей и кеша | актуальные элементы файлов |
| `POST /api/translate` | запуск единственной пакетной задачи | `job_id`; пустой пакет даёт `400`, занятый обработчик — `409` |
| `POST /api/unload` | выгрузка активной локальной модели | состояние и безопасное сообщение |
...
| пакет | `batch_size`, `threads`, `verbose` |
...
| Google | `tld`, `timeout`, `request_delay_ms` |
...
- TranslateGemma 4B требует CUDA INT8, TranslateGemma 12B — CUDA NF4 с двойным квантованием,
  Seed-X — CUDA Int4 через `compressed-tensors`. Для этих изолированных профилей запрещено
  размещение на CPU или диске. NLLB допускает CPU лишь после явного разрешения.
...
`uv.lock` хранит точный общий граф, включая `compressed-tensors==0.18.0` для Seed-X, а
`.python-version` закрепляет выбор Python 3.14. Установите
